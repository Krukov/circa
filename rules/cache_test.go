...
func (s *mockStorage) Expire(key string, ttl time.Duration) error {
	s.Called(key, ttl)
	return nil
}

func (s *mockStorage) Incr(key string) (int, error) {
	args := s.Called(key)
...
func TestCacheRule_Process(t *testing.T) {
	tests := []struct {
		...
		{"simple", &message.Request{Method: "GET"}, "cached", &message.Response{Body: []byte(`mock`), Status: 200}, false},
